...
void Demo::Render() {
	...
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.direction"), 0.0f, -2.0f, -2.0f);
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.ambient"), -0.1f, -0.1f, -0.1f);
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.diffuse"), 1.0f, 1.0f, 1.0f);
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.specular"), 1.0f, 1.0f, 1.0f);
	// point light 1
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "pointLights[0].position"), 10.0f, 10.0f, 10.0f);
	...
	// point light 2
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "pointLights[1].position"), 10.0f, 10.0f, 10.0f);
	...
	// point light 3
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "pointLights[2].position"), 10.0f, 10.0f, 10.0f);
	...
	// point light 4
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "pointLights[3].position"), 10.0f, 10.0f, 10.0f);
...
void Demo::BuildLemariDepan()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("SpecularMap_Depan.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
...
void Demo::BuildLemariSamping()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("SpecularMap_Samping.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		// format position, tex coords
		// front
		-1.5, -1.5, 0.5, 0, 0,  0.0f,  0.0f,  1.0f,  // 0
		0.5, -1.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f,   // 1
		0.5,  1.5, 0.5, 1, 1,  0.0f,  0.0f,  1.0f,   // 2
		-1.5,  1.5, 0.5, 0, 1,  0.0f,  0.0f,  1.0f,  // 3
		...
		// left
		-2.5, -1.5, -0.5, 0, 0,  -1.0f,  0.0f,  0.0f, // 12
		-2.5, -1.5,  0.5, 1, 0,  -1.0f,  0.0f,  0.0f, // 13
		-2.5,  1.5,  0.5, 1, 1,  -1.0f,  0.0f,  0.0f, // 14
		-2.5,  1.5, -0.5, 0, 1,  -1.0f,  0.0f,  0.0f, // 15
